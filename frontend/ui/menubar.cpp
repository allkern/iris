...
static void show_recent_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu(ICON_MS_HISTORY " Open Recent", iris->recents.size())) {
        ...
        if (imgui::MenuItem(ICON_MS_DELETE_HISTORY " Clear all recents")) {
            iris->recents.clear();
        }

        // To-do: Use try_open_file
        // if (imgui::MenuItem("Clear invalid recents")) {
        //     iris->recents.clear();
        // }

        // To-do
        // if (imgui::MenuItem("Stop recents history")) {
        // }
...
static void show_iris_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu("Iris")) {
        if (imgui::MenuItem(ICON_MS_DRIVE_FILE_MOVE " Open...")) {
            audio::mute(iris);
        ...
        if (imgui::MenuItem(ICON_MS_JOYSTICK " Open Arcade...")) {
            audio::mute(iris);
        ...
        if (imgui::MenuItem(iris->debug.pause ? ICON_MS_PLAY_ARROW " Run" : ICON_MS_PAUSE " Pause", "Space")) {
            iris->debug.pause = !iris->debug.pause;
        }

        // To-do: Show confirm dialog maybe?
        if (imgui::MenuItem(ICON_MS_REFRESH " Reset")) {
            ps2::reset(iris->ps2);
        }

        if (imgui::MenuItem(ICON_MS_FOLDER " Change disc...")) {
            audio::mute(iris);
        ...
        if (imgui::MenuItem(ICON_MS_EJECT " Eject disc")) {
            iris->loaded = "";

            cdvd::close(iris->ps2->cdvd);
        }

        if (imgui::MenuItem(ICON_MS_CLOSE " Close")) {
...
static void show_display_settings_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu(ICON_MS_MONITOR " Display")) {
        if (imgui::BeginMenu(ICON_MS_BRUSH " Renderer")) {
            for (int i = 0; i < 3; i++) {
                BeginDisabled(i == gs::renderer::BACKEND_SOFTWARE);

                if (imgui::MenuItem(renderer_names[i], nullptr, i == iris->renderer_backend)) {
        ...
        if (imgui::BeginMenu(ICON_MS_CROP " Scale")) {
            for (int i = 2; i <= 6; i++) {
                char buf[16]; snprintf(buf, 16, "%.1fx", (float)i * 0.5f);

                if (imgui::MenuItem(buf, nullptr, ((float)i * 0.5f) == iris->scale)) {
        ...
        if (imgui::BeginMenu(ICON_MS_ASPECT_RATIO " Aspect mode")) {
            for (int i = 0; i < 7; i++) {
                if (imgui::MenuItem(aspect_mode_names[i], nullptr, iris->aspect_mode == i)) {
                    iris->aspect_mode = i;
        ...
        if (imgui::BeginMenu(ICON_MS_FILTER " Scaling filter")) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (imgui::MenuItem(filter_names[i], nullptr, iris->filter == i)) {
                    iris->filter = i;
                }
                EndDisabled();
            }
        ...
        if (imgui::BeginMenu(ICON_MS_SCREEN_ROTATION " Rotation")) {
            const int normalized_angle = ((iris->angle % 360) + 360) % 360;
            const int rotation_index = normalized_angle / 90;

            for (int i = 0; i < 4; i++) {
                if (imgui::MenuItem(rotation_names[i], nullptr, rotation_index == i)) {
                    iris->angle = i * 90;
                }
            }
        ...
        if (imgui::BeginMenu(ICON_MS_ASPECT_RATIO " Window size")) {
            const char* sizes[] = {
                "640x480",
                "800x600",
                "960x720",
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (imgui::MenuItem(sizes[i], nullptr, selected)) {
        ...
        if (imgui::BeginMenu(ICON_MS_SYNC " Present mode")) {
            ...
            for (int i = 0; i < IM_ARRAYSIZE(settings_present_mode_names); i++) {
                if (imgui::MenuItem(settings_present_mode_names[i], nullptr, iris->present_mode == i)) {
                    iris->present_mode = i;
        ...
        if (imgui::MenuItem(ICON_MS_SPEED_2X " Integer scaling", nullptr, &iris->integer_scaling)) {
            // renderer_set_integer_scaling(iris->ctx, iris->integer_scaling);
        }

        imgui::MenuItem(ICON_MS_FLIP " Flip horizontally", nullptr, &iris->flip_x);
        imgui::MenuItem(ICON_MS_FLIP " Flip vertically", nullptr, &iris->flip_y);

        if (imgui::MenuItem(ICON_MS_FULLSCREEN " Fullscreen", "F11", &iris->fullscreen)) {
            SDL_SetWindowFullscreen(iris->window, iris->fullscreen);
        }

        if (imgui::MenuItem(ICON_MS_IMAGE " Enable shaders", nullptr, &iris->enable_shaders)) {
...
static void show_audio_settings_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu(ICON_MS_MUSIC_NOTE " Audio")) {
        ...
        imgui::MenuItem(ICON_MS_VOLUME_OFF " Mute", nullptr, &iris->audio.mute);
        imgui::MenuItem(ICON_MS_MUSIC_OFF " Mute ADMA", nullptr, &iris->audio.mute_adma);
...
static void show_settings_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu("Settings")) {
        ...
        if (imgui::MenuItem(ICON_MS_DOCK_TO_BOTTOM " Show status bar", nullptr, &iris->ui.show_status_bar)) {
            SDL_SetWindowSize(iris->window, iris->window_width, iris->window_height + get_menubar_height(iris));
        }

        if (imgui::MenuItem(ICON_MS_OPEN_IN_NEW " Open data folder")) {
            SDL_OpenURL(iris->paths.pref_path.c_str());
        }

        Separator();

        if (imgui::MenuItem(ICON_MS_MANUFACTURING " Settings...")) {
...
static void show_tools_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu("Tools")) {
        imgui::MenuItem(ICON_MS_BUILD " ImGui Demo", NULL, &iris->ui.show_imgui_demo);
        imgui::MenuItem(ICON_MS_SEARCH " Memory search", NULL, &iris->applets.memory_search.open);
        if (imgui::MenuItem(ICON_MS_PHOTO_CAMERA " Take screenshot...", "F9")) {
            audio::mute(iris);
        ...
        if (imgui::MenuItem(ICON_MS_MOVIE " Dump GS frames...")) {
...
static void show_timescale_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu(ICON_MS_MORE_TIME " Timescale")) {
        for (int i = 0; i < 9; i++) {
            char buf[16]; snprintf(buf, 16, "%dx", 1 << i);

            if (imgui::MenuItem(buf, nullptr, iris->timescale == (1 << i))) {
                iris->timescale = (1 << i);
...
static void show_debug_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu("Debug")) {
        ...
        imgui::MenuItem(ICON_MS_BUG_REPORT " Breakpoints", NULL, &iris->applets.breakpoints.open);
        imgui::MenuItem(ICON_MS_BRUSH " GS debugger", NULL, &iris->applets.gs_debugger.open);
        imgui::MenuItem(ICON_MS_MUSIC_NOTE " SPU2 debugger", NULL, &iris->applets.spu2_debugger.open);
        imgui::MenuItem(ICON_MS_MEMORY " Memory viewer", NULL, &iris->applets.memory_viewer.open);
        imgui::MenuItem(ICON_MS_VIEW_IN_AR " VU disassembler", NULL, &iris->applets.vu_disassembler.open);
        ...
        if (imgui::MenuItem(ICON_MS_SKIP_NEXT " Skip FMVs", NULL, &iris->skip_fmv)) {
...
static void show_help_menu(Instance* iris) {
    ...
    if (imgui::BeginMenu("Help")) {
        ...
        if (imgui::MenuItem(ICON_MS_EXCLAMATION " Report an issue")) {
        ...
        if (imgui::MenuItem(ICON_MS_FACT_CHECK " Report compatibility")) {
