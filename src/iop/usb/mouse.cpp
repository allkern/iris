...
static const uint8_t mouse_report_desc[REPORT_DESC_LEN] = {
...
static const uint8_t mouse_config_desc[34] = {
    ...
    REPORT_LEN, 0x00, // wMaxPacketSize
...
static int get_descriptor(Mouse* mouse, uint16_t value, uint16_t length) {
    int type = value >> 8;
    ...
    switch (type) {
        ...
        case 0x03: {
            switch (index) {
                ...
                default: {
                    iris_debug(mouse, "GET_DESCRIPTOR: unknown string index {}", index);
        ...
        default: {
            iris_debug(mouse, "GET_DESCRIPTOR: unknown type {:02x} index {}", type, index);

            return 0;
        }
    }

    // iris_debug(mouse, "GET_DESCRIPTOR type={:02x} index={} -> {} bytes (host asked {})", //     type, index, len, length);
...
static int control(device::Device* dev, int pid, uint8_t* buf, int len) {
    ...
    if (pid == device::USB_PID_SETUP) {
        ...
        // iris_debug(mouse, "SETUP bmRequestType={:02x} bRequest={:02x} wValue={:04x} wIndex={:04x} wLength={}", //     bmRequestType, bRequest, wValue, wIndex, wLength);
        ...
        if (type == 0) {
            switch (bRequest) {
                ...
                case 0x06: { // GET_DESCRIPTOR
                    mouse->ctrl_len = get_descriptor(mouse, wValue, wLength);
                } break;
                ...
                default: {
                    iris_debug(mouse, "STALL: unsupported standard request {:02x}", bRequest);
            ...
            switch (bRequest) {
                ...
                default: {
                    iris_debug(mouse, "STALL: unsupported class request {:02x}", bRequest);

                    return device::USB_ACK_STALL;
                }
            }
        } else {
            iris_debug(mouse, "STALL: unsupported request type {} (bmRequestType={:02x})", type, bmRequestType);
...
static int interrupt_in(Mouse* mouse, uint8_t* buf, int len) {
    if (!mouse->dirty) {
    ...
    // iris_debug(mouse, "EP1 IN: delivering report {:02x} {:02x} {:02x} {:02x} (n={})", //     report[0], report[1], report[2], report[3], n);
...
static int transfer(device::Device* dev, int pid, int ep, uint8_t* buf, int len) {
    ...
    if (ep == 1 && pid == device::USB_PID_IN)
        return interrupt_in((Mouse*)dev->priv, buf, len);
