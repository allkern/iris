...
static const uint8_t kbd_report_desc[REPORT_DESC_LEN] = {
...
static int get_descriptor(Kbd* kbd, uint16_t value, uint16_t length) {
    int type = value >> 8;
    ...
    switch (type) {
        ...
        case 0x03: {
            switch (index) {
                ...
                default: {
                    iris_debug(kbd, "GET_DESCRIPTOR: unknown string index {}", index);
        ...
        default: {
            iris_debug(kbd, "GET_DESCRIPTOR: unknown type {:02x} index {}", type, index);

            return 0;
        }
    }

    // iris_debug(kbd, "GET_DESCRIPTOR type={:02x} index={} -> {} bytes (host asked {})", //     type, index, len, length);
...
static int control(device::Device* dev, int pid, uint8_t* buf, int len) {
    ...
    if (pid == device::USB_PID_SETUP) {
        ...
        // iris_debug(kbd, "SETUP bmRequestType={:02x} bRequest={:02x} wValue={:04x} wIndex={:04x} wLength={}", //     bmRequestType, bRequest, wValue, wIndex, wLength);
        ...
        if (type == 0) {
            switch (bRequest) {
                ...
                case 0x06: { // GET_DESCRIPTOR
                    kbd->ctrl_len = get_descriptor(kbd, wValue, wLength);
                } break;
                ...
                default: {
                    iris_debug(kbd, "STALL: unsupported standard request {:02x}", bRequest);
            ...
            switch (bRequest) {
                ...
                default: {
                    iris_debug(kbd, "STALL: unsupported class request {:02x}", bRequest);

                    return device::USB_ACK_STALL;
                }
            }
        } else {
            iris_debug(kbd, "STALL: unsupported request type {} (bmRequestType={:02x})", type, bmRequestType);
...
static int interrupt_in(Kbd* kbd, uint8_t* buf, int len) {
    if (!kbd->report_dirty) {
    ...
    // iris_debug(kbd, "EP1 IN: delivering report {:02x} {:02x} {:02x} {:02x} {:02x} {:02x} {:02x} {:02x} (n={})", //     kbd->report[0], kbd->report[1], kbd->report[2], kbd->report[3],
...
static int transfer(device::Device* dev, int pid, int ep, uint8_t* buf, int len) {
    ...
    if (ep == 1 && pid == device::USB_PID_IN)
        return interrupt_in((Kbd*)dev->priv, buf, len);
...
void key(device::Device* dev, uint8_t usage, int pressed) {
    ...
    // iris_debug(kbd, "kbd_key usage={:02x} pressed={} (connected={} address={} config={})", //     usage, pressed, dev->connected, dev->address, dev->configuration);
