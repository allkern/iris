...
static inline uint32_t ohci_read_dword(Usb* usb, uint32_t addr) {
    return iop::bus::read32(usb->hw.bus, addr);
}

static inline void ohci_write_dword(Usb* usb, uint32_t addr, uint32_t data) {
    iop::bus::write32(usb->hw.bus, addr, data);
}

static void ohci_read_buf(Usb* usb, uint32_t addr, uint8_t* buf, int len) {
    for (int i = 0; i < len; i++)
        buf[i] = iop::bus::read8(usb->hw.bus, addr + i);
}

static void ohci_write_buf(Usb* usb, uint32_t addr, const uint8_t* buf, int len) {
    for (int i = 0; i < len; i++)
        iop::bus::write8(usb->hw.bus, addr + i, buf[i]);
}

static void ohci_update_irq(Usb* usb) {
    if ((usb->hc_interrupt_enable & OHCI_INTR_MIE) &&
        (usb->hc_interrupt_status & usb->hc_interrupt_enable & 0x7fffffff)) {
        iop::intc::irq(usb->hw.intc, iop::intc::USB);
    }
}

static void ohci_set_interrupt(Usb* usb, uint32_t bit) {
...
static void ohci_update_rhsc(Usb* usb) {
...
static device::Device* ohci_find_device(Usb* usb, int addr) {
    for (int i = 0; i < OHCI_NUM_PORTS; i++) {
...
static int ohci_service_td(Usb* usb, uint32_t ed_addr, uint32_t* ed) {
    ...
    if (dir == 1) {
        pid = device::USB_PID_OUT;
    } else if (dir == 2) {
        pid = device::USB_PID_IN;
    } else {
        pid = dp;
    }
    ...
    device::Device* dev = ohci_find_device(usb, fa);

    int result;

    if (!dev) {
        // iris_debug(usb, "TD {:08x}: no device at address {} (ep={} pid={})", td_addr, fa, en, pid);
        result = device::USB_ACK_NODEV;
    } else if (pid == device::USB_PID_IN) {
        result = device::transfer(dev, pid, en, temp, buf_len);
    } else {
        if (buf_len)
    ...
    if (result == device::USB_ACK_NAK)
        return 0;

    int cc = OHCI_CC_NOERROR;
    int actual = 0;

    if (result == device::USB_ACK_NODEV) {
        cc = OHCI_CC_DEVICENOTRESP;
    } else if (result == device::USB_ACK_STALL) {
        cc = OHCI_CC_STALL;
    } else {
        actual = result;

        if (pid == device::USB_PID_IN && actual > 0) {
...
static void ohci_service_ed_list(Usb* usb, uint32_t ed_addr) {
    int guard = 0;
...
static void ohci_schedule_frame(Usb* usb);
...
static void ohci_frame(void* udata, int overshoot) {
    ...
    if (usb->done_queue && !(usb->hc_interrupt_status & OHCI_INTR_WDH)) {
        ...
        // iris_debug(usb, "writeback done head {:08x}, raising WDH (intr_enable={:08x})", //     done & ~0xf, usb->hc_interrupt_enable);
...
static void ohci_schedule_frame(Usb* usb) {
    ...
    scheduler::schedule(usb->hw.sched, event);
}

static void ohci_soft_reset(Usb* usb) {
...
static void ohci_port_write(Usb* usb, int port, uint32_t data) {
    ...
    // iris_debug(usb, "write RhPortStatus[{}] = {:08x} (status was {:08x})", port, data, *ps);
    ...
    if ((data & OHCI_PORT_PRS) && (*ps & OHCI_PORT_CCS)) {
        ...
        *ps |= OHCI_PORT_PES | OHCI_PORT_PRSC;
        ...
        // iris_debug(usb, "port {} reset", port);
    }
...
void set_port_device(Usb* usb, int port, int type) {
    ...
    device::Device* dev = &usb->device[port];
    ...
    // iris_debug(usb, "port {} device set to {} (status={:08x})", port, usb_device_types[type].name, usb->hc_rh_port_status[port]);
}
...
Usb* create(logger::Logger* logger, iop::intc::Intc* intc, iop::bus::Bus* bus, scheduler::Scheduler* sched) {
    ...
    usb->logger = logger;
    usb->logger_id = logger::register_source(logger, "usb");
...
uint64_t read32(Usb* usb, uint32_t addr) {
    ...
    if (addr >= USB_HC_RHPORTSTATUS && addr < USB_HC_RHPORTSTATUS + OHCI_NUM_PORTS * 4) {
        ...
        // iris_debug(usb, "read RhPortStatus[{}] = {:08x}", port, usb->hc_rh_port_status[port]);

        return usb->hc_rh_port_status[port];
    }

    // iris_debug(usb, "unhandled read at {:02x}", addr);
...
void mouse_move(Usb* usb, int dx, int dy, int dz) {
    for (int i = 0; i < OHCI_NUM_PORTS; i++) {
        if (usb->device_type[i] == USB_DEVICE_MOUSE) {
            mouse::move(&usb->device[i], dx, dy, dz);
        }
    }
}
...
void mouse_button(Usb* usb, int button, int pressed) {
    for (int i = 0; i < OHCI_NUM_PORTS; i++) {
        if (usb->device_type[i] == USB_DEVICE_MOUSE) {
            mouse::button(&usb->device[i], button, pressed);
        }
    }
}
