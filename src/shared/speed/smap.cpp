...
static void smap_phy_write(Smap* smap, int reg, uint16_t data) {
    switch (reg) {
...
static void smap_tx_irq_event(void* udata, int overshoot) {
    Smap* smap = (Smap*)udata;

    send_irq(smap->speed, SMAP_INTR_TXEND | SMAP_INTR_TXDNV);
}

static void smap_transmit(Smap* smap) {
    ...
    while (smap->tx_bd[smap->tx_bd_index % SMAP_BD_MAX_ENTRY].ctrl & SMAP_BD_TX_READY) {
        Bd* bd = &smap->tx_bd[smap->tx_bd_index % SMAP_BD_MAX_ENTRY];
    ...
    scheduler::schedule(smap->speed->sched, event);
}

// EMAC3 (IBM EMAC core)
static uint32_t smap_emac3_read(Smap* smap, uint32_t reg) {
    switch (reg) {
    ...
    iris_debug(smap, "Unhandled EMAC3 read reg {:02x}", reg);

    return 0;
}

static void smap_emac3_write(Smap* smap, uint32_t reg, uint32_t data) {
    switch (reg) {
        case SMAP_R_EMAC3_MODE0: {
        ...
        case SMAP_R_EMAC3_TxMODE0: {
            // iris_debug(smap, "EMAC3 TxMODE0 write {:08x}", data);
        ...
        case SMAP_R_EMAC3_STA_CTRL: {
            // iris_debug(smap, "EMAC3 STA_CTRL write {:08x}", data);
    ...
    iris_debug(smap, "Unhandled EMAC3 write reg {:02x} ({:08x})", reg, data);
}

static Bd* smap_bd_ptr(Smap* smap, uint32_t off) {
    uint32_t boff = off - SMAP_BD_REGBASE;
    uint32_t index = boff / sizeof(Bd);
...
static uint32_t smap_reg_read(Smap* smap, uint32_t off) {
    switch (off) {
    ...
    iris_debug(smap, "Unhandled reg read off={:04x}", off);

    return 0;
}

static void smap_reg_write(Smap* smap, uint32_t off, uint32_t data) {
    switch (off) {
    ...
    iris_debug(smap, "Unhandled reg write off={:04x} ({:08x})", off, data);
}

Smap* create(logger::Logger* logger) {
    Smap* smap = new Smap();

    smap->logger = logger;
    smap->logger_id = logger::register_source(logger, "smap");
...
uint64_t read8(Smap* smap, uint32_t addr) {
    return smap_reg_read(smap, addr - SMAP_REGOFF);
}

uint64_t read16(Smap* smap, uint32_t addr) {
    ...
    if (off >= SMAP_BD_REGBASE) {
        Bd* bd = smap_bd_ptr(smap, off);

        if (!bd)
            return 0;

        uint16_t v = *(uint16_t*)((uint8_t*)bd + ((off - SMAP_BD_REGBASE) % sizeof(Bd)));
...
uint64_t read32(Smap* smap, uint32_t addr) {
    uint32_t off = addr - SMAP_REGOFF;
    ...
    if (off == SMAP_R_RXFIFO_DATA || off == SMAP_R_FIFO_DATA)
        return fifo_read(smap);

    return smap_reg_read(smap, off);
}

void write8(Smap* smap, uint32_t addr, uint64_t data) {
    smap_reg_write(smap, addr - SMAP_REGOFF, (uint8_t)data);
}

void write16(Smap* smap, uint32_t addr, uint64_t data) {
    uint32_t off = addr - SMAP_REGOFF;
    ...
    if (off >= SMAP_BD_REGBASE) {
        ...
        if (bd)
            *(uint16_t*)((uint8_t*)bd + ((off - SMAP_BD_REGBASE) % sizeof(Bd))) =
...
void write32(Smap* smap, uint32_t addr, uint64_t data) {
    uint32_t off = addr - SMAP_REGOFF;
...
int dma_pending(Smap* smap) {
    return ((smap->txfifo_ctrl | smap->rxfifo_ctrl) & SMAP_TXFIFO_DMAEN) != 0;
}

void fifo_write(Smap* smap, uint32_t data) {
...
int receive(Smap* smap, const uint8_t* buf, int len) {
    ...
    Bd* bd = &smap->rx_bd[smap->rx_bd_index % SMAP_BD_MAX_ENTRY];
    ...
    send_irq(smap->speed, SMAP_INTR_RXEND);

    return 1;
}

void destroy(Smap* smap) {
    delete smap;
}
