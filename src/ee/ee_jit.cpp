...
std::unordered_map <int, asmjit::ujit::Gp> g_reg_cache;

static inline asmjit::ujit::Gp ee_get_reg(struct ee_state* ee, int i, asmjit::ujit::UniCompiler* uc, asmjit::ujit::Gp ee_ptr, bool load = true) {
    using namespace asmjit;

    auto it = g_reg_cache.find(i);

    if (it == g_reg_cache.end()) {
        ujit::Mem r_ptr = ujit::mem_ptr(ee_ptr, offsetof(ee_state, r) + (i * sizeof(uint128_t)));
        ujit::Gp gp = uc->new_gp64();

        if (load) uc->load_u64(gp, r_ptr);
...
static inline void ee_flush_regs(struct ee_state* ee, asmjit::ujit::UniCompiler* uc, asmjit::ujit::Gp ee_ptr) {
    using namespace asmjit;

    for (std::pair <int, ujit::Gp> p : g_reg_cache) {
        ujit::Mem r_ptr = ujit::mem_ptr(ee_ptr, offsetof(ee_state, r) + (p.first * sizeof(uint128_t)));

        if (p.first) uc->store_u64(r_ptr, p.second);
    }

    g_reg_cache.clear();
}

static inline void ee_emit_instruction(struct ee_state* ee, struct ee_instruction& i, uint32_t pc, asmjit::ujit::Gp ee_ptr, asmjit::ujit::UniCompiler* uc) {
    using namespace asmjit;

    // if ((uintptr_t)i.func == (uintptr_t)ee_i_mfc0) {
    //     printf("mfc0\n");

    //     ujit::Mem cr_ptr = ujit::mem_ptr(ee_ptr, offsetof(ee_state, cop0_r) + (i.rd * sizeof(uint32_t)));
    //     ujit::Gp v = uc->new_gp64();
    //     ujit::Gp rt = ee_get_reg(ee, i.rt, uc, ee_ptr);

    //     uc->load_u32(v, cr_ptr);
    //     uc->mov(rt, v);
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_sll) {
//         printf("sll\n");

//         if (!i.rd) return;

//         ujit::Gp rd = ee_get_reg(ee, i.rd, uc, ee_ptr, false);
//         ujit::Gp rt = ee_get_reg(ee, i.rt, uc, ee_ptr);

//         uc->shl(rd, rt, asmjit::imm(i.sa));
...
//         uc->cc->movsxd(rd, rt);
//     #endif
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_slti) {
//         printf("slti\n");

//         if (!i.rt) return;

//         ujit::Gp rs = ee_get_reg(ee, i.rs, uc, ee_ptr);
//         ujit::Gp rt = ee_get_reg(ee, i.rt, uc, ee_ptr, false);
//         asmjit::Imm imm = asmjit::imm((int64_t)i.i16);
...
//         uc->cc->setl(rt);
//     #endif
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_bne) {
//         printf("bne pc=%08x\n", pc);

//         ujit::Mem pc_ptr = ujit::mem_ptr(ee_ptr, offsetof(ee_state, pc));
...
//         ujit::Gp rs = ee_get_reg(ee, i.rs, uc, ee_ptr);
//         ujit::Gp rt = ee_get_reg(ee, i.rt, uc, ee_ptr);
//         asmjit::Imm imm = asmjit::imm(pc + (int32_t)(((int16_t)i.i16) << 2));

//         printf("pc=%08x\n", pc + (int32_t)(((int16_t)i.i16) << 2));
...
//         uc->store_u32(pc_ptr, tmp);
//         uc->bind(l1);
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_lui) {
//         uc->comment("lui");
//         printf("lui\n");

//         ujit::Gp rt = ee_get_reg(ee, i.rt, uc, ee_ptr, false);

//         uc->mov(rt, Imm((int64_t)(i.i16 << 16)));
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_ori) {
//         uc->comment("ori");
//         printf("ori\n");
...
//         if (i.rt != i.rs) uc->mov(rt, rs);
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_jr) {
//         printf("jr\n");

//         ujit::Mem pc_ptr = ujit::mem_ptr(ee_ptr, offsetof(ee_state, pc));
//         ujit::Gp rs = ee_get_reg(ee, i.rs, uc, ee_ptr);

//         uc->store_u32(pc_ptr, rs);
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_sync) {
//         printf("sync\n");
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_mtc0) {
//         uc->comment("mtc0");
//         printf("mtc0\n");
...
//         uc->store_u32(cr_ptr, rt);
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_addiu) {
//         uc->comment("addiu");
//         printf("addiu\n");
...
//         if (!i.rs) {
//             uc->mov(rt, asmjit::imm((int64_t)(int16_t)i.i16));
//         } else {
//             ujit::Gp rs = ee_get_reg(ee, i.rs, uc, ee_ptr);
//             asmjit::Imm imm = asmjit::imm((int64_t)(int16_t)i.i16);
...
// #endif   
//         }
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_sw) {
//         uc->comment("sw");
//         printf("sw\n");
...
//         uc->cc->invoke(
//             Out(invoke_node),
//             asmjit::imm((void*)bus_write32),
//             FuncSignature::build<struct ee_state*, uint32_t, uint64_t>()
//         );

//         invoke_node->set_arg(0, (uintptr_t)ee);
//         invoke_node->set_arg(1, tmp);
//         invoke_node->set_arg(2, rt);
//     } else if ((uintptr_t)i.func == (uintptr_t)ee_i_tlbwi) {
//         printf("tlbwi\n");
...
//         uc->store_u32(pc_ptr, tmp);
    ...
    //         Out(invoke_node),
    //         imm((void*)i.func),
    //         FuncSignature::build<void, struct ee_state*, struct ee_instruction*>()
    //     );

    //     invoke_node->set_arg(0, ee);
    //     invoke_node->set_arg(1, &i);
    // }
}
...
static inline struct ee_block* ee_cache_block(struct ee_state* ee, int max_cycles) {
    ...
    printf("---------------------\nexecuting function at pc=%08x\n", ee->pc);

    // Signature of the generated function.
    using Func = void (*)(struct ee_state* ee, struct ee_instruction* i);
    ...
    ujit::BackendCompiler backend_cc(&code);
    ujit::UniCompiler uc(&backend_cc, rt.cpu_features(), rt.cpu_hints());

    // Begin a function of the required signature (this exactly matches the Compiler use).
    FuncNode* func = uc.add_func(FuncSignature::build<void, struct ee_state*, struct ee_instruction*>());

    ujit::Gp ee_ptr = uc.new_gp_ptr();
    ujit::Gp i_ptr = uc.new_gp_ptr();

    func->set_arg(0, ee_ptr);
    func->set_arg(1, i_ptr);
    ...
    while (max_cycles) {
        i = ee_decode(bus_read32(ee, pc));

        pc += 4;

        ee_emit_instruction(ee, i, pc, ee_ptr, &uc);
    ...
    ee_flush_regs(ee, &uc, ee_ptr);

    uc.end_func();
    Error err1 = uc.finalize();
...
int ee_run_block(struct ee_state* ee, int max_cycles) {
    ee_cache_block(ee, max_cycles);
    // ee_block* blk = ee_find_block(ee, ee->pc);

    // if (!blk) {
    //     blk = ee_cache_block(ee, max_cycles);
    // }
}
