...
const size_t rom0_table_size = sizeof(rom0_table) / sizeof(rom0_table[0]);

static const Info rom1_table[] = {
...
const size_t rom1_table_size = sizeof(rom1_table) / sizeof(rom1_table[0]);
...
Info search(Type type, uint8_t* rom, size_t size) {
    ...
    md5::init(&md5);
    md5::update(&md5, rom, size);
    md5::finalize(&md5);

    std::string hash;

    for (uint8_t byte : md5.digest)
        hash += fmt::format("{:02x}", byte);

    const Info* table = (type == Type::ROM0) ? rom0_table : rom1_table;
    const size_t count = (type == Type::ROM0) ? rom0_table_size : rom1_table_size;
    ...
    std::memcpy(info.md5, hash.c_str(), sizeof(info.md5));

    return info;
}

bool is_valid(Type type, uint8_t* rom, size_t size) {
    Info info = search(type, rom, size);

    return std::strcmp(info.version, "Unknown") != 0;
}
