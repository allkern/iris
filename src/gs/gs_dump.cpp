...
inline constexpr int STATE_VERSION = 9;

inline constexpr int REGSTATE_SIZE =
    4              /* version                     */ +
...
inline constexpr int PRIVREGS_SIZE = 0x2000;
...
static inline void write_data(Dump* dump, const void* data, size_t size) {
    fwrite(data, 1, size, dump->file);
}

static inline void write_u8(Dump* dump, uint8_t v) {
    fwrite(&v, sizeof(v), 1, dump->file);
}

static inline void write_u32(Dump* dump, uint32_t v) {
    fwrite(&v, sizeof(v), 1, dump->file);
}

static inline void write_u64(Dump* dump, uint64_t v) {
    fwrite(&v, sizeof(v), 1, dump->file);
}

static void write_register_state(Dump* dump, gs::Gs* gs) {
    write_u32(dump, STATE_VERSION);
...
static void write_priv_registers(Dump* dump, gs::Gs* gs) {
    gs::PrivilegedState state;

    gs::get_privileged_state(gs, &state);

    uint8_t buf[PRIVREGS_SIZE];
...
bool begin(Dump* dump, const char* path,
    gs::Gs* gs, gif::Gif* gif,
    ...
    header.state_size = REGSTATE_SIZE + vram_size + 4 * (16 + 4) + 4;
    header.serial_offset = sizeof(Header);
    header.serial_size = serial_size;
    header.crc = crc;
    header.screenshot_width = 0;
    header.screenshot_height = 0;
    header.screenshot_offset = sizeof(Header) + serial_size;
    header.screenshot_size = 0;

    uint32_t header_size = sizeof(Header) + serial_size;
...
void vsync(Dump* dump, gs::Gs* gs) {
    ...
    write_u8(dump, PACKET_PRIVREGISTERS);
    write_priv_registers(dump, gs);

    write_u8(dump, PACKET_VSYNC);
